...
export interface AIGenerationMetadata {
  ...
  cost: number;
  // High-resolution generation metadata for Runware
...
export interface InstagramCarouselCanvas extends Canvas {
  ...
  // Canvas size properties for persistence (separate from format defaults)
  customSize?: {
    width: number;
    height: number;
  };
...
export const createInstagramCarouselProject = (
  ...
  // Create initial canvases - either from provided slides or at least one default canvas
  let initialCanvases: InstagramCarouselCanvas[] = [];
  
  if (slides.length > 0) {
    // AI-generated project with slides
    initialCanvases = slides.map((slide: any, index: number) =>
      createCarouselCanvas(slide, index + 1, index === 0)
    );
  } else {
    // New empty project - create one default canvas for immediate canvas management access
    initialCanvases = [
      createCarouselCanvas({
        title: `${name} - Slide 1`,
        content: 'Start editing your carousel content here...',
        backgroundPrompt: 'Instagram post background, modern design, clean layout'
      }, 1, true)
    ];
  }

  const slideCount = initialCanvases.length;

  return {
    id: `carousel_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
    name,
    description: originalPrompt ? `Instagram carousel generated from: "${originalPrompt.substring(0, 100)}..."` : 'Instagram carousel project',
    type: 'instagram-carousel',
    ...
    userId,
    isPublic: false,
    tags: ['instagram', 'carousel', originalPrompt ? 'ai-generated' : 'manual'],
    carouselMetadata: {
      originalPrompt,
      backgroundStrategy,
      ...
      slideCount,
      maxSlideCount: 10,
      isRegeneratable: !!originalPrompt // Only regeneratable if AI-generated
    }
  };
};
