...
export const generateThumbnail = async (
  ...
  const uint8Data = data instanceof Uint8Array ? new Uint8Array(Array.from(data)) : new TextEncoder().encode(data as string);
...
export const convertToWebM = async (
  ...
  const data = await ffmpeg.readFile(outputName);
  const uint8Data = data instanceof Uint8Array ? new Uint8Array(Array.from(data)) : new TextEncoder().encode(data as string);
  const blob = new Blob([uint8Data], { type: "video/webm" });
...
export const extractAudio = async (
  ...
  const data = await ffmpeg.readFile(outputName);
  const uint8Data = data instanceof Uint8Array ? new Uint8Array(Array.from(data)) : new TextEncoder().encode(data as string);
  const blob = new Blob([uint8Data], { type: `audio/${format}` });
